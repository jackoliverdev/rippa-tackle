...
import { createUserProfile, createUser, User } from "@/lib/user-service";
...
export const SignUpForm: FC<SignUpFormProps> = ({ onShowLogin, onSignUp }) => {
  ...
  const signUp = async ({
    ...
    try {
      ...
      // Step 2: Create user in Supabase users table
      const supabaseUser = await createUser({
        firebase_uid: user.uid,
        email: user.email || email,
        // Optionally add first_name, last_name, profile_image_url, role
      });

      if (!supabaseUser || !supabaseUser.id) {
        throw new Error("Failed to create user in Supabase. Please try again.");
      }

      toast({
        title: "Account created!",
        description: "Welcome to Rippa Tackle! You are now signed in.",
      });
      
      router.push('/account');
    } catch (error: any) {
      if (error.code && error.code.includes("already")) {
        toast({ title: "Email already in use", description: "Please sign in instead or use a different email address." });
      } else {
